...
@section('title', 'Proyek Riset Baru - Kepala Divisi')
...
<div class="min-h-screen bg-[#ECE8E5] text-black relative overflow-hidden">
    ...
    <div class="relative z-10 mx-auto max-w-4xl px-6 py-12 space-y-8">
        <div class="select-none space-y-2">
            <p class="text-xs uppercase tracking-[0.32em] text-gray-500">Inisiasi Proyek Divisi</p>
            <h1 class="text-3xl sm:text-4xl font-semibold tracking-[0.24em] text-[#1f1f1f]">Proyek Baru</h1>
            <p class="text-sm text-gray-600">Rumuskan proyek riset yang menjadi acuan bagi operatif dalam divisi {{ $division?->name ?? 'Anda' }}.</p>
        </div>

        <div class="rounded-3xl border border-white/40 bg-white/20 p-8 shadow-2xl backdrop-blur-xl">
            <form action="{{ route('division-projects.store') }}" method="post" class="space-y-6">
                @csrf
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <label class="block text-left">
                        ...
                        @php
                            ...
                            if ($oldStatus && !$statusOptions->contains($oldStatus)) {
                                $statusOptions = $statusOptions->prepend($oldStatus);
                            }
                        @endphp
                    ...
                    <label class="block text-left">
                        ...
                        <input type="date" name="initiated_at" value="{{ old('initiated_at') }}"
                            class="glass-field mt-2 w-full rounded-2xl border border-white/40 bg-white/30 px-4 py-3 text-sm tracking-wide text-[#1f1f1f] shadow-sm focus:border-[#EA7645] focus:outline-none focus:ring-2 focus:ring-[#EA7645]/20">
                        @error('initiated_at')
                            <span class="mt-1 block text-xs text-[#EA7645] tracking-wide">{{ $message }}</span>
                        @enderror
                ...
                <label class="block text-left">
                    ...
                    <textarea name="objective" rows="6" class="glass-field custom-scroll mt-2 w-full rounded-3xl border border-white/40 bg-white/30 px-5 py-4 text-sm leading-relaxed tracking-wide text-[#1f1f1f] shadow-sm focus:border-[#EA7645] focus:outline-none focus:ring-2 focus:ring-[#EA7645]/20">{{ old('objective') }}</textarea>
                    @error('objective')
                        <span class="mt-1 block text-xs text-[#EA7645] tracking-wide">{{ $message }}</span>
                    @enderror
                </label>
