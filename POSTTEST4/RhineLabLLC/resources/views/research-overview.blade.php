        ...
        @if($primaryUser)
            @php
                $divisionSummaries = $divisions->map(function ($division) {
                    $recordsCount = $division->projects->sum(function ($project) {
                        return $project->records->count();
                    });

                    $members = $division->projects->flatMap(function ($project) {
                        return $project->records->pluck('user');
                ...
                $aggregatedMembers = $divisionSummaries->flatMap(function ($summary) {
            ...
            <section class="rounded-2xl border border-[#d6cdc5] bg-white/85 px-6 py-7 shadow-sm">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
                        ...
                        <div class="flex items-end gap-6 md:gap-8">
                            ...
                            <div class="text-right space-y-1">
                                ...
                                <p class="text-lg font-bold text-[#48AA8C]">{{ $aggregatedMembers->count() }}</p>
                            </div>
                            <div class="text-right space-y-1">
                ...
                <div id="division-panel-hub" class="division-panel space-y-8">
                    @if($aggregatedMembers->isNotEmpty())
                        <div class="rounded-xl border border-[#e2d9d1] bg-white/70 p-5">
                            ...
                            <div class="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                                @foreach($aggregatedMembers as $member)
                                    <div class="division-member-card flex items-center justify-between gap-3 rounded-xl border border-[#d4cbc4] bg-white/85 px-4 py-3">
                                        <div class="flex items-center gap-3">
                                            <div class="h-10 w-10 overflow-hidden rounded-full border border-white shadow">
                                            ...
                                            <div class="space-y-0.5">
                                                ...
                                                <p class="text-[10px] uppercase tracking-[0.24em] text-[#48AA8C]">{{ $member->position ?? 'Member' }}</p>
                                            </div>
                                        </div>
                    ...
                    @foreach($divisionSummaries as $summary)
                        @php
                            ...
                            $divisionMembers = $summary['members'];
                        @endphp
                        ...
                        <section class="rounded-2xl border border-[#e2d9d1] bg-gradient-to-br from-white via-[#f9f6f2] to-[#f1ebe5] px-5 py-6 shadow-sm">
                            <header class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                <div class="flex items-start gap-3">
                                    <div class="h-14 w-14 overflow-hidden rounded-2xl border border-[#e2d9d1] bg-white/80 shadow-sm">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-sm text-gray-600">Lead Scientist: {{ $division->lead_scientist }}</p>
                                    </div>
                                </div>
                                <div class="flex gap-6 md:gap-8">
                                    <div class="text-right space-y-1">
                                        <p class="text-xs uppercase tracking-[0.3em] text-gray-500">Anggota</p>
                                        <p class="text-base font-bold text-[#48AA8C]">{{ $divisionMembers->count() }}</p>
                                    </div>
                                    <div class="text-right space-y-1">
                                        <p class="text-xs uppercase tracking-[0.3em] text-gray-500">Rekaman Riset</p>
